...
defined('JPATH_VM_ADMINISTRATOR') or define('JPATH_VM_ADMINISTRATOR', JPATH_ROOT.DS.'administrator'.DS.'components'.DS.'com_directcron');
...
if (!defined('_VM_SCRIPT_INCLUDED')) {
	...
	/**
	 * DirectCron custom installer class
	 */
	class com_directcronInstallerScript {
...
// 			$this->path = JInstaller::getInstance()->getPath('extension_administrator');
		...
		public function checkIfUpdate(){
			...
			$update = false;
			$db = JFactory::getDBO();
			$q = 'SHOW TABLES LIKE "%directcron_adminmenuentries%"'; //=>jos_directcron_shipment_plg_weight_countries
			$db->setQuery($q);
			if($db->loadResult()){

				$q = "SELECT count(id) AS idCount FROM `#__directcron_adminmenuentries`";
				$db->setQuery($q);
		...
		public function preflight ($type, $parent=null) {
			...
			if(version_compare(JVERSION,'1.6.0','ge')) {
				...
				$q = 'DELETE FROM `#__menu` WHERE `menutype` = "main" AND
						(`link`="index.php?option=com_directcron" OR `alias`="directcron" )';
				$db = JFactory::getDbo();
				$db -> setQuery($q);
				$db -> query();
		...
		public function install ($loadVm = true) {
			...
			//			if(!class_exists('DirectCronModelUpdatesMigration')) require(JPATH_VM_ADMINISTRATOR.DS.'models'.DS.'updatesMigration.php');
			$params = JComponentHelper::getParams('com_languages');
			$lang = $params->get('site', 'en-GB');//use default joomla
			$lang = strtolower(strtr($lang,'-','_'));

			$model = JModel::getInstance('updatesmigration', 'DirectCronModel');
			...
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'shipment');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'payment');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'category');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'category'.DS.'resized');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'manufacturer');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'manufacturer'.DS.'resized');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'product');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'product'.DS.'resized');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'forSale');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'forSale'.DS.'resized');
			$this->createIndexFolder(JPATH_ROOT .DS. 'images'.DS.'stories'.DS.'directcron'.DS.'typeless');


			$dst = JPATH_ROOT .DS. 'images' .DS. 'stories' .DS. 'directcron';
			...
			//include($this->path.DS.'install'.DS.'install.directcron.html.php');

			// perhaps a redirect to updatesMigration here rather than the html file?
			//			$parent->getParent()->setRedirectURL('index.php?option=com_directcron&view=updatesMigration');
		...
		public function update ($loadVm = true) {
			...
			if(!class_exists('DirectCronModelUpdatesMigration')) require(JPATH_VM_ADMINISTRATOR . DS . 'models' . DS . 'updatesmigration.php');
			$model = new DirectCronModelUpdatesMigration(); //JModel::getInstance('updatesmigration', 'DirectCronModel');
			$model->execSQLFile($this->path.DS.'install'.DS.'install.sql',$lang);
...
/*			$table = '#__directcron_customs';
			$fieldname = 'field_type';
			$fieldvalue = 'G';
			$this->addToRequired($table,$fieldname,$fieldvalue,"INSERT INTO `#__directcron_customs`
...
*/
			...
			$this->deleteReCreatePrimaryKey('#__directcron_userinfos','directcron_userinfo_id');
...
// 			$query = 'ALTER TABLE `#__directcron_userinfos`  CHANGE COLUMN `directcron_userinfo_id` `directcron_userinfo_id` INT(1) NOT NULL AUTO_INCREMENT FIRST';
// 			$this->_db->setQuery($query);
// 			if(!$this->_db->query()){

// 			} else {
// 				$query = 'ALTER TABLE `#__directcron_userinfos` AUTO_INCREMENT = 1';
// 				$this->_db->setQuery($query);
// 			}
		...
		*/
		...
		private function checkAddDefaultShoppergroups(){
			...
			$q = 'SELECT `directcron_shoppergroup_id` FROM `#__directcron_shoppergroups` WHERE `default` = "1" ';
			...
			if(empty($res)){
				$q = "INSERT INTO `#__directcron_shoppergroups` (`directcron_shoppergroup_id`, `directcron_vendor_id`, `shopper_group_name`, `shopper_group_desc`, `default`, `shared`) VALUES
								(NULL, 1, '-default-', 'This is the default shopper group.', 1, 1);";
				$db->setQuery($q);
				$db->query();
			}

			$q = 'SELECT `directcron_shoppergroup_id` FROM `#__directcron_shoppergroups` WHERE `default` = "2" ';
		...
		private function changeShoppergroupDataSetAnonShopperToOne(){
			...
			$q = 'SELECT * FROM `#__directcron_shoppergroups` WHERE directcron_shoppergroup_id = "1" ';
			$this->_db->setQuery($q);
			...
			if($sgroup['default']!=2){
				...
				$stdgroup = null;
				$stdgroup = array('directcron_shoppergroup_id' => 1,
									'directcron_vendor_id'	=> 1,
				...
				$sgroup['directcron_shoppergroup_id'] = 0;
				$table = new TableShoppergroups($db);
		...
		private function updateUnit(  $field, $UnitMigrateValues) {
			...
			foreach ($UnitMigrateValues as $old => $new) {
				$query = 'UPDATE  `#__directcron_products` SET `'.$field.'` = "' . $new . '" WHERE  `'.$field.'` = "' . $old.'" ';
				$this->_db->setQuery($query);
				if (!$this->_db->query()) {
			...
			if (!$ok) return false;
			$query = 'SHOW COLUMNS FROM `#__directcron_products` ';
			$this->_db->setQuery($query);
			$columns = $this->_db->loadResultArray(0);
			if(!in_array($field,$columns)){
				$query = "ALTER TABLE  `#__directcron_products` CHANGE  `".$field."`  `".$field."` VARCHAR( 3 ) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT  'Kg'";
				$this->_db->setQuery($query);
				if(!$this->_db->query()){
					vmError('Install updateUnit '.$field.' '.$this->_db->getErrorMsg() );
					return false;
		...
		private function updateAdminMenuEntry() {
			...
			$query = 'UPDATE `#__directcron_adminmenuentries` SET `name`="COM_VIRTUEMART_SHIPMENTMETHOD_S", `view`="shipmentmethod" WHERE `id`="16" LIMIT 1';
			$this->db->setQuery($query);
			$this->db->query($query);

			$q = 'SELECT `id` FROM `#__directcron_adminmenuentries` WHERE `view` = "creditcard" ';
			$this->db->setQuery($q);
			$res = $this->db->loadResult();
			if($res){
				$query = 'DELETE FROM `#__directcron_adminmenuentries` WHERE `view`="creditcard" LIMIT 1;';
				$this->db->setQuery($query);
				$this->db->query($query);
			}
		...
		private function renamePsPluginTables($tablenames){
			...
			foreach($tablenames as $key => $name){
				$query = 'SHOW TABLES LIKE "%_directcron_order_shipper_'.$name.'%"'; //=>jos_directcron_shipment_plg_weight_countries
				$this->_db->setQuery($query);
				if($this->_db->loadResult()){
					$query = 'ALTER TABLE `#__directcron_order_shipper_'.$name.'` RENAME TO `#__directcron_'.$key.'_plg_'.$name.'`';
					$this->_db->setQuery($query);
					$this->_db->query();
				}
			}
		...
		private function migrateCustomPluginTableIntoCustoms(){
			...
			$query = 'SHOW TABLES LIKE "%directcron_customplugins%"';
			$this->_db->setQuery($query);
			if($this->_db->loadResult()){
				$q = 'SELECT * FROM `#__directcron_customplugins` ';
				$this->_db->setQuery($q);
				...
				if(!empty($items) and count($items)>0){
					...
					if(!$error){
						$q = 'DROP TABLE `#__directcron_customplugins` ';
						$this->_db->setQuery($q);
						$this->_db->query();
					}
				}
		...
		private function updateJParamsToVmParams(){
			...
			foreach($tablenames as $name){
				$query = 'SHOW TABLES LIKE "%directcron_'.$name.'methods"';
				$this->_db->setQuery($query);
				if($this->_db->loadResult()){
					$q = 'SELECT `directcron_'.$name.'method_id`,`'.$name.'_params` FROM `#__directcron_'.$name.'methods` ';
					...
					foreach($items as $item){
						if(strpos("\n",$item[$name.'_params'])!==false and strpos("|",$item[$name.'_params'])===false){
							...
							$q = 'UPDATE `#__directcron_'.$name.',methods` SET `'.$name.'_params`='.$str.' WHERE `directcron_'.$name.'method_id`="'.$item['directcron_'.$name.'method_id'].'" ';
							$this->_db->setQuery($q);
							if(!$this->_db->query()){
		...
		public function uninstall ($parent=null) {
			...
			$this->loadVm();
			include($this->path.DS.'install'.DS.'uninstall.directcron.html.php');
		...
		public function postflight ($type, $parent=null) {
			if ($type != 'uninstall') {
				...
				$db = JFactory::getDBO();
				$q = 'SHOW TABLES LIKE "%directcron_configs%"'; //=>jos_directcron_shipment_plg_weight_countries
				$db->setQuery($q);
				$res = $db->loadResult();
				if(!empty($res)){
					JRequest::setVar(JUtility::getToken(), '1', 'post');
					$config = JModel::getInstance('config', 'DirectCronModel');
			...
			// 			if(!JFile::exists(JPATH_VM_ADMINISTRATOR.DS.'directcron_defaults.cfg')){
			// 				JFile::copy('directcron_defaults.cfg-dist','directcron_defaults.cfg',JPATH_VM_ADMINISTRATOR);
			// 			}
		...
		public function displayFinished($update){
			...
			$lang->load('com_directcron.sys',JPATH_ADMINISTRATOR,'en_GB',true);
			$lang->load('com_directcron',JPATH_ADMINISTRATOR,'en_GB',true);

			//load specific language
			$lang->load('com_directcron.sys',JPATH_ADMINISTRATOR,null,true);
			$lang->load('com_directcron',JPATH_ADMINISTRATOR,null,true);
			?>
<link
	rel="stylesheet"
	href="components/com_directcron/assets/css/install.css"
	type="text/css" />
<link
	rel="stylesheet"
	href="components/com_directcron/assets/css/toolbar_images.css"
	type="text/css" />
	...
	<table
		...
		<tr>
			<td
				valign="top"
				align="center"><a
				href="http://directcron.net"
				target="_blank"> <img
					border="0"
					align="center"
					src="components/com_directcron/assets/images/vm_menulogo.png"
					alt="Cart" /> </a> <br /> <br />
				<h2>
			...
			<td>
				...
				if($update){
					...
					/*					if($this->renewConfigManually){
						echo '<br />'.JText::_('When you got an error deleting the directcron.cfg file <br />
					Delete this file manually (administrator/components/com_directcron/directcron.cfg) and please use
				...
				if(!$update){
					...
					<div class="icon">
						<a
							href="<?php echo JROUTE::_('index.php?option=com_directcron&view=updatesmigration&task=installSampleData&token='.JUtility::getToken()) ?>">
		...
		<?php } ?>
				...
				<div class="icon">
				<a href="<?php echo JROUTE::_('index.php?option=com_directcron&task=disableDangerousTools&token='.JUtility::getToken() ) ?>">
...
<?php
	...
	function com_install() {
		$vmInstall = new com_directcronInstallerScript();
