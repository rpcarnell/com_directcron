...
require_once (CRONFRAME_ROOT.DS.'thirdparty'.DS.'pear'.DS.'Parser.php');
...
class BloggieTemplate extends JObject
{
	...
	{
		$value = "css";//$this->_themeParams->get($name, $default);
		return $value;
	}
	...
	function getSetting($variable, $default=null)
	{
		if ($variable)
		{
			return "css";//$this->_settings->get($variable, $default);
		}
		return $default;
	}
		...
		switch ($this->_type)
		{
			...
			case 'drc:if':
				...
				if ($countinue){
					...
					$else_node =& $this->get_child_by_tag('drc:else');
